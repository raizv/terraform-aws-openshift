...
# Insert 15 second delay so AWS credentials definitely available at all AWS endpoints
...
module "openshift" {
  # source	= "github.com/raizv/terraform-aws-openshift"
...
#   provisioner "remote-exec" {
#     inline = [
#       "scp -o StrictHostKeyChecking=no -i ~/.ssh/private-key.pem centos@${module.openshift.master_public_dns}:~/.kube/config ~/config",
#     ]
...
#   provisioner "local-exec" {
#     command = "scp -o StrictHostKeyChecking=no -i private-key.pem  centos@${module.openshift.bastion_public_dns}:~/config config"
#   }
...
#   provisioner "remote-exec" {
#     inline = [
#       "scp -o StrictHostKeyChecking=no -i ~/.ssh/private-key.pem centos@${module.openshift.master_public_dns}:~/vault-reviewer-token vault-reviewer-token",
#     ]
...
#   provisioner "local-exec" {
#     command = "scp -o StrictHostKeyChecking=no -i private-key.pem  centos@${module.openshift.bastion_public_dns}:~/vault-reviewer-token vault-reviewer-token"
#   }
